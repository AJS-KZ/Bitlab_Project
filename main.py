...
token = '' # here should be a token id
...
def getFoodByFoodType(id):
    global mydb
    mycursor = mydb.cursor()
    sql = "SELECT f.id, f.name, f.price, f.description, f.food_type_id, f.restaurant_id, ft.name as foodType, " \
          "r.name as restaurantName FROM foods f " \
          "LEFT OUTER JOIN food_types ft ON ft.id = f.food_type_id " \
          "LEFT OUTER JOIN restaurants r ON r.id = f.restaurant_id WHERE f.food_type_id = "+str(id)
    mycursor.execute(sql)
    result = mycursor.fetchall()
    return result

def getFoodByRestaurantId(id):
    global mydb
    mycursor = mydb.cursor()
...
def handle_text(message):
    ...
    if message.text.lower() == "/start":
        ...
        if menu == "main":
            ...
            text = '==================\n'
            for food in foods:
                text = text + str(food[0]) + ") " + food[1] + " " + str(food[2]) + " - KZT | from " + food[7] + "\n"
            bot.send_message(message.chat.id, text)

        elif menu == "choose_by_restaurant":
            menu = "choose_food"
            id = message.text.lower()
            foods = getFoodByRestaurantId(id)
            text = '==================\n'
            for food in foods:
